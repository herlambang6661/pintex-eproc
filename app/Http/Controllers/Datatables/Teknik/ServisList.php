...
class ServisList extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (($row->edited == 0 || $row->edited == null) && $row->status == 'PROSES PERSETUJUAN') {
                        $btnEdit = '<a href="#" class="dropdown-item edit-btn" data-bs-toggle="modal" data-bs-target="#modalEditServis" data-id="' . $row->id . '">
                    ...
                    if ($row->status == "PROSES PERSETUJUAN" && (Auth::user()->alias == 'pur' || Auth::user()->alias == 'kng' || Auth::user()->alias == 'own')) {
                        $btnCondition = '
                                    <a class="remove dropdown-item" href="javascript:void(0);" data-id="' . $row->kodeseri_servis . '" data-nama="' . $row->namaBarang . '" data-desc="' . $row->keterangan . '">
                    ...
                    $btn = '<div class="btn-list flex-nowrap">
                                <form method="POST" action="printServis" target="_blank">
                                    <input type="hidden" name="_token" value="' . csrf_token() . '">
                                    <input type="hidden" name="noform" value="' . $row->noformservis . '">
                                ...
                                <div class="dropdown-menu dropdown-menu-end" style="">
                                    ...
                                    <a href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDetailServis" data-id="' . $row->id . '" data-noform="' . $row->noformservis . '">
    ...
    {
        DB::table('servisitm')->where('kodeseri_servis', '=', $id)->delete();
